...
public sealed class InventoryRepository
{
    ...
    public InventoryRepository(IEnumerable<InventoryItem> items)
    {
        ArgumentNullException.ThrowIfNull(items);
        _items = items.ToList();
    }

    public InventoryRepository() : this(DefaultInventory()) { }
    ...
    /// </summary>
    private static List<InventoryItem> DefaultInventory()
    {
        return new List<InventoryItem>
        {
            new(Guid.NewGuid(), "Da Hong Pao (Big Red Robe) – Wuyi Rock Oolong", 15.85m, 42, new StarRating(3)),
            new(Guid.NewGuid(), "Tie Guan Yin (Iron Goddess) – Anxi Oolong", 17.68m, 137, new StarRating(1)),
            new(Guid.NewGuid(), "Dragon Well (Longjing) – Premium Green", 29.90m, 8, new StarRating(5)),
            new(Guid.NewGuid(), "Bi Luo Chun (Green Snail Spring) - Green", 12.36m, 163, new StarRating(2)),
            ...
            new(Guid.NewGuid(), "Huangshan Mao Feng (Yellow Mountain) - Green Tea", 14.99m, 144,
                new StarRating(5)),
            ...
            new(Guid.NewGuid(), "Phoenix Dan Cong Ya Shi (Duck Poop) - Oolong", 124.99m, 100, new StarRating(5)),
            new(Guid.NewGuid(), "Dian Hong (Yunnan Golden Tips) - Black Tea", 85.99m, 0, new StarRating(4)),
